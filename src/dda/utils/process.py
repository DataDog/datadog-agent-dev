...
class SubprocessRunner:
    ...
    def static_capture(
        ...
        **kwargs: Any,
        ...
        """
        Run a command and capture its output.
        Equivalent to [`SubprocessRunner.capture`][dda.utils.process.SubprocessRunner.capture] but can be used in a static context (i.e. when the `Application` object is not yet available).

        /// warning
        This method is much less instrumented than [`capture`][dda.utils.process.SubprocessRunner.capture] and should only be used when the `Application` object is not yet available.
        ///
        ...
        Other parameters:
            **kwargs: Additional keyword arguments to pass to the `subprocess.run` call.
        """

        if kwargs.get("show", False):
            message = "Output cannot be shown when capturing statically, as the Application is not yet initialized."
            raise RuntimeError(message)
        ...
        command, kwargs = SubprocessRunner.__sanitize_arguments(command, **kwargs)
