...
@dynamic_command(short_help="Restart a QA environment")
@option_env_type()
...
def cmd(app: Application, *, env_type: str, instance: str) -> None:
    ...
    if status.state != expected_state:
        app.abort(
            f"Cannot restart QA environment `{instance}` of type `{env_type}` in state `{status.state}`, "
